...
class Article():
    ...
    def _generate(self):
        ...
        """
        self.title = fake.sentence(nb_words=6, variable_nb_words=True) if random.random() < self.probs["title"] else None
        self.subtitle = fake.sentence(nb_words=10, variable_nb_words=True) if random.random() < self.probs["subtitle"] else None
        self.corpus = " ".join(fake.paragraphs(nb=random.randrange(10, 20))) if random.random() < self.probs["corpus"] else None
        self.author = fake.name() if random.random() < self.probs["author"] else None
